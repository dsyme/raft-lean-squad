...
merging pull requests, and writing two short comments across 58
autonomous agent runs.

The agent autonomously researched the codebase, selected 26 verification
targets, designed a twenty-layer proof architecture (data structures, quorum arithmetic,
protocol operations, cross-module composition, Raft safety, reachability,
election model, correspondence validation, read-only operations, backward-scan
optimisation, flow control, multi-step reachability, persistent-storage bridge,
configuration invariants, uncommitted-state flow control, progress tracking,
raft-log append, broadcast lifecycle, next-entries slicing, and has-next-entries
boolean companion), wrote all specifications, and completed all proofs:
673~theorems across 73~Lean~4 files (stdlib only, no Mathlib),
...
reachability model.  An additional 26 correspondence-validation files
provide 587 compile-time \texttt{\#guard} assertions and 18+ Rust test
functions, confirming that the Lean models agree with the Rust
implementation on concrete inputs across all major targets.

The total LLM token cost was approximately \$840 (120~runs at
...
the twenty-layer proof architecture, specification writing, or any actual
proof engineering.  His total intervention over the project's 120~runs
...
\item The entire effort is \emph{automated}: every target selection,
  ...
  LLM-driven FV agent across 88 independent runs, with only two brief
...
runs: the twenty-layer architecture, the choice of targets, and the
...
\midrule
1 & Data Structures     & 7  & 195 \\
2 & Quorum Arithmetic   & 7  & 139 \\
3 & Protocol Operations & 7  & 111 \\
4 & Cross-Module Composition & 3 & 27 \\
5 & Raft Safety         & 2  & 20  \\
6 & Reachability        & 2  & 11  \\
7 & Election Model (complete) & 14 & 175 \\
8 & Correspondence Validation & 26 & 4+587\texttt{g} \\
9 & ReadOnly Operations & 1  & 14  \\
10& FindConflictByTerm  & 1  & 10  \\
11& Flow Control        & 1  & 18  \\
12& Configuration Invariants & 1 & 11 \\
13& Multi-step Reachability  & 1  & 7  \\
14& Persistent Storage Bridge & 2 & 16 \\
15& Uncommitted State        & 1  & 18 \\
16& Progress Tracking        & 1  & 26 \\
17& Raft-Log Append          & 1  & 16 \\
18& Broadcast Lifecycle      & 1  &  3 \\
19& Next-Entries Slicing     & 1  &  7 \\
20& Has-Next-Entries Boolean  & 1  & 14 \\
\midrule
  & \textbf{Total}      & \textbf{73} & \textbf{673+587\texttt{g}} \\
\bottomrule
\end{tabular}
\caption{Proof layers and theorem counts. Layer 8 files contain 587 \texttt{\#guard} assertions (\texttt{g}) evaluated at compile time; they contribute 4 named theorems (in \texttt{FindConflictCorrespondence.lean}).}
\label{tab:layers}
\end{table}
...
independent confirmation.  Together, the 587 Lean \texttt{\#guard}
...
\texttt{LimitSize.lean} (25~theorems) formalises the Rust function
...
\texttt{Inflights.lean} (50 theorems) is the largest single file.  It
...
\texttt{Progress.lean} (37 theorems) formalises the per-node progress
...
Five files prove correctness of core Raft log operations.
\texttt{MaybeAppend.lean} (19 theorems) gives a complete post-condition
...
\texttt{RaftLogAppend.lean} (19~theorems) formalises \texttt{raft\_log.rs::append}
as a Lean 4 function over a \texttt{RaftLog} record, proving
...
Twenty-one correspondence-test files provide compile-time evidence that the
...
The 21 files cover: \texttt{find\_conflict} (27~guards, exact),
\texttt{maybe\_append} (35~guards, exact), \texttt{is\_up\_to\_date}
(14~guards, exact), \texttt{committed\_index} (13~guards, abstraction),
\texttt{limit\_size} (12~guards, abstraction), \texttt{config\_validate}
(14~guards, exact), \texttt{inflights} (14~guards, abstraction),
\texttt{log\_unstable} (14~guards, exact), \texttt{tally\_votes}
(12~guards, exact), \texttt{vote\_result} (17~guards, exact),
\texttt{has\_quorum} (17~guards, exact),
\texttt{read\_only} (16~guards, exact),
\texttt{find\_conflict\_by\_term} (19~guards, abstraction),
\texttt{progress} (55~guards, abstraction --- \texttt{maybeUpdate},
\texttt{maybeDecrTo}, \texttt{optimisticUpdate}, and state transitions),
\texttt{maybe\_persist} (23~guards, abstraction --- \texttt{maybe\_persist}
and \texttt{maybe\_persist\_snap} with snapshot fixtures),
\texttt{maybe\_commit} (19~guards, abstraction --- \texttt{maybe\_commit}
and \texttt{commit\_to}: the A6 term-safety guard and monotone commit),
\texttt{raft\_log\_append} (24~guards, abstraction --- all three
\texttt{truncate\_and\_append} branches: empty-log, truncate-only, and
truncate-and-append), and
\texttt{maybe\_persist\_fui} (28~guards, abstraction --- FUI derivation
under snap, no-snap+entries, and no-snap+empty-entries cases),
\texttt{uncommitted\_state} (13~guards, abstraction --- \texttt{maybeIncrease}
and \texttt{maybeReduce} cases),
\texttt{has\_next\_entries} (33~guards, abstraction --- \texttt{hasNextEntriesSince}
and \texttt{hasNextEntries} entry-existence guards), and
\texttt{next\_entries} (20~guards, abstraction --- \texttt{nextEntriesSince}
and \texttt{nextEntries} slice model).
In total: 587 compile-time correspondence assertions and 18+ Rust test functions.
...
\texttt{FindConflictByTerm.lean} (10~theorems) formalises the
\texttt{find\_conflict\_by\_term} function (\texttt{src/raft\_log.rs\#L218--L257}),
...
\texttt{ConfigurationInvariants.lean} (11~theorems) formalises the
voter/learner disjointness invariant across joint-consensus configuration
changes.  Key theorems: CI5 (\texttt{allVoters} $\cap$ learners = $\emptyset$
...
\paragraph{Layer~13: Multi-step Reachability.}
\texttt{MultiStepReachability.lean} (7~theorems) extends the single-step
reachability model to $n$-step chains.  Key theorem MS2 provides an
end-to-end safety certificate: given a valid $n$-step AE sequence from a
\texttt{RaftReachable} state, the resulting state is also
\texttt{RaftReachable} and satisfies the safety invariant.

\paragraph{Layer~14: Persistent-Storage Bridge.}
...
\paragraph{Layer~15: Uncommitted-State Flow Control.}
\texttt{UncommittedState.lean} (18~theorems) formalises the
\texttt{UncommittedState} struct (\texttt{src/raft.rs}) responsible for
flow-controlling uncommitted-log byte usage.  Key theorems: US16
(complete characterisation of budget rejection), US17 (dual: acceptance
...
\paragraph{Layer~16: Progress Tracking.}
...
\paragraph{Layer~17: Raft-Log Append.}
\texttt{RaftLogAppend.lean} (16~theorems, RA1--RA9 + P4--P7) formalises
\texttt{RaftLog::truncate\_and\_append} (\texttt{src/raft\_log.rs}).
...
\paragraph{Layer~18: Broadcast Lifecycle.}
\texttt{BroadcastLifecycle.lean} (3~theorems, BL1--BL3) connects the
\texttt{BroadcastSeq} inductive structure to \texttt{RaftReachable}.
BL1 proves that any broadcast sequence yields a valid AE list; BL2 that
applying a broadcast sequence to a \texttt{RaftReachable} state preserves
reachability; and BL3 that a broadcast from \texttt{initialCluster}
produces a reachable state.  Together these close the remaining gap
between the election broadcast chain and the main reachability predicate.

\paragraph{Layer~19: Next-Entries Slicing.}
...
\paragraph{Layer~20: Has-Next-Entries Boolean Companion.}
\texttt{HasNextEntries.lean} (14~theorems, HNE1--HNE14) formalises the
boolean predicate \texttt{hasNextEntriesSince} / \texttt{hasNextEntries}.
HNE7 characterises when the predicate is false (empty slice); HNE8--HNE14
characterise the \texttt{hasNextEntriesSince} family; HNE14 gives the
...
Total: 673 theorems, 73~files (45 proof files + 26 correspondence-test files + 2 shared + 1 lakefile),
0 \texttt{sorry}, $\approx$21{,}000 lines of Lean.
...
FindConflictCorrespondence &  4 & 27 \texttt{\#guard}, exact \\
MaybeAppendCorrespondence &  0 & 35 \texttt{\#guard}, exact \\
IsUpToDateCorrespondence  &  0 & 14 \texttt{\#guard}, exact \\
CommittedIndexCorrespondence & 0 & 13 \texttt{\#guard}, abstraction \\
LimitSizeCorrespondence   &  0 & 12 \texttt{\#guard}, abstraction \\
ConfigValidateCorrespondence & 0 & 14 \texttt{\#guard}, exact \\
InflightsCorrespondence   &  0 & 14 \texttt{\#guard}, abstraction \\
LogUnstableCorrespondence &  0 & 14 \texttt{\#guard}, exact \\
TallyVotesCorrespondence  &  0 & 12 \texttt{\#guard}, exact \\
VoteResultCorrespondence  &  0 & 17 \texttt{\#guard}, exact \\
HasQuorumCorrespondence   &  0 & 17 \texttt{\#guard}, exact \\
ReadOnlyCorrespondence    &  0 & 16 \texttt{\#guard}, exact \\
FindConflictByTermCorrespondence & 0 & 19 \texttt{\#guard}, abstraction \\
ProgressCorrespondence    &  0 & 55 \texttt{\#guard}, abstraction \\
ProgressTrackerCorrespondence & 0 & 47 \texttt{\#guard}, abstraction \\
MaybePersistCorrespondence &  0 & 23 \texttt{\#guard}, abstraction \\
MaybeCommitCorrespondence  &  0 & 19 \texttt{\#guard}, abstraction \\
RaftLogAppendCorrespondence & 0 & 24 \texttt{\#guard}, abstraction \\
MaybePersistFUICorrespondence & 0 & 28 \texttt{\#guard}, abstraction \\
ElectionCorrespondence    &  0 & 28 \texttt{\#guard}, abstraction \\
ConfigurationInvariantsCorrespondence & 0 & 21 \texttt{\#guard}, abstraction \\
JointVoteCorrespondence   &  0 & 20 \texttt{\#guard}, abstraction \\
QuorumRecentlyActiveCorrespondence &  0 & 16 \texttt{\#guard}, abstraction \\
...
NextEntriesCorrespondence &  0 & 20 \texttt{\#guard}, abstraction \\
UncommittedStateCorrespondence & 0 & 13 \texttt{\#guard}, abstraction \\
\midrule
\textbf{Total}    & \textbf{673} & \textbf{0 sorry; 587 \texttt{\#guard}} \\
\bottomrule
\end{tabular}
\caption{Full file inventory (all at Phase~5, 0 sorry). Correspondence files contribute \texttt{\#guard} assertions but no named theorems.}
\label{tab:inventory}
\end{table*}
...
All 673~theorems are consistent with the Rust implementation, as further
confirmed by 587 compile-time \texttt{\#guard} correspondence assertions
...
The twenty-layer architecture ensures that every theorem in upper layers has
...
26 FV targets, the twenty-layer proof architecture, all 26 informal
specifications, all Lean formalisations, all 673 proofs, the 26
...
Across 120 runs spanning $\approx$17 weeks, the agent produced:
\begin{itemize}\setlength\itemsep{0pt}
  \item 26 informal specification documents
  \item 73 Lean~4 source files ($\approx$21{,}000 lines)
  \item 673 machine-checked theorems
  \item 587 compile-time \texttt{\#guard} correspondence assertions
...
infrastructure.  The twenty-layer architecture emerged organically from
...
being prescribed.  The eighth layer (correspondence validation) emerged
when the agent recognised the need to ground abstract Lean models in
concrete Rust behavior.
...
Finding no implementation bugs in 673~theorem-checks is itself evidence
...
Each of the 88 Lean Squad runs consumed approximately \$7 of LLM
token cost at Claude Sonnet~4.6 pricing (at time of publication), for a total of
approximately~\$603.  This covers the full pipeline: reading
...
decisions---stdlib-only (no Mathlib), the twenty-layer decomposition, the
...
\emph{diverges}: the Lean Squad agent completed all 673~theorems
...
Squad accumulates knowledge across 88 independent GitHub Actions runs
...
Our $\approx$15{,}000 lines of Lean and 581 theorems are modest by comparison.
...
90 LLM agent runs (each $\sim$15--30 minutes of wall-clock time)
plus two one-line human comments.  If the approach scales---and the
successful progression from Layer~1 through Layer~8 suggests it
...
a twenty-layer proof architecture, wrote all specifications and proofs,
and produced 673 machine-checked theorems across 73 files in 120 runs,
...
reachability model, and an additional 26 correspondence-validation
files providing 587 compile-time \texttt{\#guard} assertions and 18+
...
Future work on Lean Squad includes closing the final concrete protocol
connection (composing the AEBroadcastInvariant through the full Raft
election lifecycle to complete the unconditional concrete safety proof),
...
specifications, all 673 proofs, the twenty-layer architecture, and the
...
\texttt{sorry}.  The proof artefacts, agent configuration, and full run

...
targets, designed a ten-layer proof architecture (data structures, quorum arithmetic,
protocol operations, cross-module composition, Raft safety, reachability,
election model, correspondence validation, read-only operations, and backward-scan
optimisation), wrote all specifications, and completed all proofs:
575~theorems across 55~Lean~4 files (stdlib only, no Mathlib),
...
reachability model.  An additional 18 correspondence-validation files
provide 373 compile-time \texttt{\#guard} assertions and 18 Rust test
functions, confirming that the Lean models agree with the Rust
implementation on concrete inputs across all major targets.

The total LLM token cost was approximately \$603 (88~runs at
...
the ten-layer proof architecture, specification writing, or any actual
proof engineering.  His total intervention over the project's 88~runs
...
runs: the ten-layer architecture, the choice of targets, and the
...
\midrule
1 & Data Structures     & 5  & 159 \\
2 & Quorum Arithmetic   & 7  & 139 \\
3 & Protocol Operations & 5  & 75  \\
4 & Cross-Module Composition & 3 & 27 \\
5 & Raft Safety         & 2  & 20  \\
6 & Reachability        & 1  & 4   \\
7 & Election Model (complete) & 11 & 122 \\
8 & Correspondence Validation & 18 & 4+373\texttt{g} \\
9 & ReadOnly Operations & 1  & 14  \\
10& FindConflictByTerm  & 1  & 10  \\
\midrule
  & \textbf{Total}      & \textbf{55} & \textbf{575+373\texttt{g}} \\
\bottomrule
\end{tabular}
\caption{Proof layers and theorem counts. Layer 8 files contain 373 \texttt{\#guard} assertions (\texttt{g}) evaluated at compile time; they contribute 4 named theorems (in \texttt{FindConflictCorrespondence.lean}).}
\label{tab:layers}
\end{table}
...
To ground the abstract models, we developed 18 \emph{correspondence-test}
...
independent confirmation.  Together, the 373 Lean \texttt{\#guard}
...
Eighteen correspondence-test files provide compile-time evidence that the
...
The 18 files cover: \texttt{find\_conflict} (27~guards, exact),
...
In total: 373 compile-time correspondence assertions and 18 Rust test functions.
...
Total: 581 theorems, 56~files (38 proof files + 18 correspondence-test files),
0 \texttt{sorry}, $\approx$15{,}000 lines of Lean.
...
Progress          & 37 & State-machine WF invariant \\
...
ConcreteTransitions & 11 & CT4: LMI preserved; CT5: broadcast $\to$ match \\
CommitRule        &  9 & CR8: \texttt{hnew\_cert} closed \\
MaybeCommit       & 12 & MC4: A6 term safety \\
MaybePersist      & 13 & MP6: async-persist below firstUpdateIndex; MP8: idempotent \\
...
ElectionBroadcastChain &  6 & EBC4: broadcast seq $\Rightarrow$ HaeCovered; EBC6: full chain \\
MaybePersistFUI   &  8 & FU1: snap blocks advance; FU8: fui $>$ persisted \\
RaftLogAppend     & 19 & RA1--RA9, P4--P7: append spec \\
...
\midrule
\textbf{Total}    & \textbf{581} & \textbf{0 sorry; 373 \texttt{\#guard}} \\
\bottomrule
\end{tabular}
...
All 575~theorems are consistent with the Rust implementation, as further
confirmed by 373 compile-time \texttt{\#guard} correspondence assertions
...
The ten-layer architecture ensures that every theorem in upper layers has
...
26 FV targets, the ten-layer proof architecture, all 26 informal
specifications, all Lean formalisations, all 575 proofs, the 18
...
Across 90 runs spanning $\approx$14 weeks, the agent produced:
\begin{itemize}\setlength\itemsep{0pt}
  \item 26 informal specification documents
  \item 56 Lean~4 source files ($\approx$15{,}000 lines)
  \item 581 machine-checked theorems
  \item 373 compile-time \texttt{\#guard} correspondence assertions
...
infrastructure.  The ten-layer architecture emerged organically from
...
Finding no implementation bugs in 575~theorem-checks is itself evidence
...
decisions---stdlib-only (no Mathlib), the ten-layer decomposition, the
...
\emph{diverges}: the Lean Squad agent completed all 575~theorems
...
a ten-layer proof architecture, wrote all specifications and proofs,
and produced 581 machine-checked theorems across 56 files in 90 runs,
...
reachability model, and an additional 18 correspondence-validation
files providing 373 compile-time \texttt{\#guard} assertions and 18
...
specifications, all 575 proofs, the ten-layer architecture, and the
correspondence-validation harness were produced autonomously by the AI
agent.  All theorems are machine-checked by Lean~4.28.0 with zero

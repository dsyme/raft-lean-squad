...
eleven tasks.  Two tasks are drawn and communicated to the agent;
...
The eleven tasks, in order of the typical progression, are:
...
\item \textbf{Correspondence Validation.}
  Write compile-time \texttt{\#guard} tests that evaluate the Lean model
  on concrete inputs and check the result against the Rust implementation.
  Optionally use Aeneas for mechanical extraction; otherwise write
  manual test fixtures with matching Rust \texttt{\#[test]} functions.
...
4  & Implementation    &  6.0 & 2.0 \\
5  & Proof Assistance  & 12.0 & 3.0 \\
6  & Correspondence Review & 10.0 & 3.0 \\
7  & Critique          &  4.0/10.0 & 2.0/3.0 \\
8  & Corr.\ Validation  & 12.0 & 5.0/3.0 \\
9  & CI                & 12.0 & 2.0 \\
10 & Report            &  3.0/8.0 & 2.0/3.0 \\
11 & Paper             &  2.0/8.0 & 1.0/3.0 \\
\bottomrule
\end{tabular}
\caption{Phase-weighted task selection (11~tasks).  ``Early'' weights
  apply when the task's prerequisites are met but the task has not yet been
  completed; ``late'' weights apply once the artefact exists.
  Slash notation (e.g.\ 4.0/10.0) indicates the weight while proofs are
  incomplete vs.\ after proof maturity.  Tasks~5, 8, and~9 are most
  heavily weighted during active proof work.}
\label{tab:weights}
\end{table}
...
(Table~\ref{tab:layers}).  Additional targets were added as
the agent discovered cross-module composition opportunities,
protocol-level safety properties,
a reachability model, and a complete election model.
...
tracker state machine, proving that the well-foundedness invariant
...
for \texttt{maybe\_append}: the prefix is untouched
and the suffix is correctly applied, providing
a full behavioural specification of AppendEntries log modification.

\texttt{FindConflict.lean} (12 theorems) proves that the function
returns zero iff all entries match, and returns the first mismatch index
otherwise.

\texttt{MaybePersist.lean} (13 theorems) formalises the async-persist
safety guards, proving a complete iff-characterisation of the three
guards (monotonicity, firstUpdateIndex bound, term-match) and
idempotence (a second call with the same arguments always returns false).

\paragraph{Layer~4: Cross-Module Composition.}
Three files prove properties spanning multiple independent modules.
The key result is that if two voters
...
A second file proves that
\texttt{maybe\_append} never advances a committed index beyond what
the current quorum has certified.
...
AppendEntries preserves the Log Matching Invariant and
the No-Rollback property, formalising the Raft protocol transition
...
proving that \texttt{leaderCommit} reflects a genuine quorum
acknowledgement and \emph{closing} the
\texttt{hnew\_cert} hypothesis of \texttt{RaftReachable.step}.

\texttt{MaybeCommit.lean} (12~theorems) formalises the commit-index
advance function and proves \emph{term safety}: the commit index
advances only when the entry at the new committed index has
the leader's current term, formalising Raft~\S5.4.2.
This is the formal proof of the mechanism that prevents the ``figure~8''
problem from the Raft paper.

\texttt{ConcreteProtocolStep.lean} (14~theorems) introduces the
\texttt{ValidAEStep} structure and proves the \emph{A5 bridge}:
if a state is \texttt{RaftReachable} and a valid AppendEntries step fires,
the resulting state is also \texttt{RaftReachable}.  This is
the first theorem that directly connects a concrete Raft message to the
abstract reachability model.

Four additional files close the \texttt{hqc\_preserved} gap
(the hardest hypothesis) through a chain: election safety
($\leq 1$ leader per term) $\to$ vote-granting implies
\texttt{isUpToDate} $\to$ broadcast-sequence structural induction
$\to$ \texttt{hqc\_preserved} from broadcast $\to$ election
lifecycle bridge.

\texttt{ElectionLifecycle.lean} (7~theorems) is the capstone
file.  It defines \texttt{ElectionEpoch}---a
...
\texttt{RaftReachable} state.
\texttt{fullProtocolStep\_safe}
is the end-to-end result: no abstract axioms remain.

Two additional files formalise
raft-log append (prefix preservation, committed-index monotonicity)
and the \texttt{firstUpdateIndex} derivation for async-persist
safety (the key invariant ensuring \texttt{maybe\_persist} cannot
advance past stable storage).
...
file evaluates the Lean model function on concrete inputs and checks
the result against the expected value from the Rust implementation
via \texttt{\#guard} assertions, which are evaluated
by \texttt{lake build} at compile time.  Matching Rust test functions run
the same cases via \texttt{cargo test}.
In total: 625 compile-time correspondence assertions across 28 files
(12 at ``exact'' correspondence, 16 at ``abstraction'' level)
and 18+ Rust test functions.
The full per-file breakdown is available in the repository's
\texttt{REPORT.md}.

\paragraph{Layers~9--20: Standalone Module Verifications.}
Twelve additional layers (170~theorems across 14~files) verify
individual Raft subsystems that do not feed into the safety chain:
read-only request handling, backward-scan conflict resolution,
flow-control byte accounting, configuration invariants,
persistent-storage bridging, progress tracking, raft-log append,
next-entries slicing, composed progress sets, in-memory storage,
batch concatenation validity, and snapshot restoration.
Each file is fully proved with 0~\texttt{sorry};
Table~\ref{tab:layers} gives per-layer file and theorem counts.
...
\item \texttt{hlogs'}: discharged by \texttt{ValidAEStep}, which models
  single-voter AppendEntries.
\item \texttt{hno\_overwrite}: discharged via the committed-$\leq$-prev
  precondition and the concrete-transitions bridge.
\item \texttt{hqc\_preserved}: discharged by the election lifecycle
  chain, which combines election safety, the broadcast induction,
  and the election lifecycle bridge.
\item \texttt{hcommitted\_mono}: discharged from
  \texttt{ValidAEStep}.
\item \texttt{hnew\_cert}: discharged by the commit rule
  together with term safety (the commit index
  advances only for entries from the current leader's term).
\end{itemize}

The key theorem is \texttt{fullProtocolStep\_safe}: given any
...
proceeds: election safety (at most one leader per term) $\to$
vote-granting implies \texttt{isUpToDate} $\to$
\texttt{BroadcastSeq} induction $\to$
\texttt{hqc\_preserved} from broadcast $\to$
election lifecycle bridge.
...
\texttt{hno\_overwrite}.  The \texttt{RaftProtocol} layer
...
\texttt{fullProtocolStep\_safe}, the end-to-end safety result.
...
\begin{itemize}\setlength\itemsep{0pt}
\item The quorum intersection theorem is the \emph{only} mathematical
  fact about quorums used in the safety proof.  If it were wrong, the proof would
  fail.
\item \texttt{MaybeAppend} gives a complete behavioural
  ...
  one of these theorems to fail.
\item \texttt{SafetyComposition} is the first theorem that cannot
...
The standalone modules, while not in the safety chain, verify the
...
though the safety proof abstracts over it. At the conclusion of the work, the author asked an agent to analyze why
the 25 standalone files were not linked into larger results. The analysis
reported \texttt{fullProtocolStep\_safe} proves that no two nodes apply
different entries at the same committed index---a property that depends
only on quorum intersection, log-matching preservation, and the commit
rule.  It does \emph{not} depend on how the log is stored
(\texttt{Inflights}, \texttt{LogUnstable}, \texttt{MemStorage}), how
entries are delivered to the application (\texttt{NextEntries}), flow
control (\texttt{UncommittedState}, \texttt{ProgressTracker}), or
read-only request handling (\texttt{ReadOnly}).  Three groups could,
however, strengthen the safety chain:

\begin{itemize}\setlength\itemsep{0pt}
\item The \emph{persistence chain} (45 theorems) could yield a
  crash-recovery safety theorem if \texttt{RaftReachable} were extended
  with a crash/restore transition.
\item \texttt{CommitRule} + \texttt{MaybeCommit} +
  \texttt{CrossModuleComposition} (28 theorems) prove the same properties
  the main chain assumes via \texttt{ValidAEStep} fields, but from the
  Rust function models---linking them would give a concrete function-level
  discharge, reducing the trust boundary.
\item \texttt{TallyVotes} + \texttt{JointTally} + \texttt{JointVote}
  (67 theorems) could derive election uniqueness from the concrete
  vote-counting algorithm, strengthening \texttt{RE5} from an assumed
  property of \texttt{ElectionEpoch} to a proved consequence of the
  tally logic.
\end{itemize}

\noindent None of these extensions would change the \emph{statement} of EL7, but
they would narrow the gap between the abstract model and the Rust code.
...
closing the final gap and fully discharging all
five \texttt{RaftReachable.step} hypotheses.

What is remarkable is not that four interventions occurred,
...
The workflow's eleven-task structure with adaptive weights means the
...
\paragraph{Not all theorems feed the top-level result.}
...
a global safety theorem.  This mirrors human FV practice:
...
theorem proving.  LeanDojo~\cite{yang2023leandojo} introduced a
...
difficult problems.
Song~et~al.~\cite{song2023towards} explored LLMs as interactive
...
bridge connecting concrete AppendEntries steps to the abstract
...
workflow, merging PRs, and writing four brief GitHub issue comments.
...
and made four brief interventions; all 26 target selections, all

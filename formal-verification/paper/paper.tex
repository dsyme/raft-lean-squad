...
targets, designed a twelve-layer proof architecture (data structures, quorum arithmetic,
...
optimisation, flow control, and multi-step reachability), wrote all specifications, and completed all proofs:
647~theorems across 66~Lean~4 files (stdlib only, no Mathlib),
...
reachability model.  An additional 24 correspondence-validation files
provide 524 compile-time \texttt{\#guard} assertions and 18+ Rust test
...
the twelve-layer proof architecture, specification writing, or any actual
...
runs: the twelve-layer architecture, the choice of targets, and the
...
8 & Correspondence Validation & 24 & 4+524\texttt{g} \\
9 & ReadOnly Operations & 1  & 14  \\
10& FindConflictByTerm  & 1  & 10  \\
11& Flow Control        & 1  & 18  \\
12& Configuration Invariants & 1 & 11 \\
\midrule
  & \textbf{Total}      & \textbf{66} & \textbf{647+524\texttt{g}} \\
\bottomrule
\end{tabular}
\caption{Proof layers and theorem counts. Layer 8 files contain 524 \texttt{\#guard} assertions (\texttt{g}) evaluated at compile time; they contribute 4 named theorems (in \texttt{FindConflictCorrespondence.lean}).}
\label{tab:layers}
\end{table}
...
To ground the abstract models, we developed 24 \emph{correspondence-test}
...
independent confirmation.  Together, the 524 Lean \texttt{\#guard}
...
In total: 524 compile-time correspondence assertions and 18+ Rust test functions.
...
Total: 647 theorems, 66~files (42 proof files + 24 correspondence-test files),
0 \texttt{sorry}, $\approx$19{,}000 lines of Lean.
...
ProgressTracker   & 24 & PT1--PT24: all\_wf invariant preservation \\
...
ProgressTrackerCorrespondence & 0 & 37 \texttt{\#guard}, abstraction \\
...
\midrule
\textbf{Total}    & \textbf{647} & \textbf{0 sorry; 524 \texttt{\#guard}} \\
\bottomrule
\end{tabular}
...
All 647~theorems are consistent with the Rust implementation, as further
confirmed by 524 compile-time \texttt{\#guard} correspondence assertions
...
The twelve-layer architecture ensures that every theorem in upper layers has
...
26 FV targets, the twelve-layer proof architecture, all 26 informal
specifications, all Lean formalisations, all 647 proofs, the 24
...
Across 108 runs spanning $\approx$17 weeks, the agent produced:
\begin{itemize}\setlength\itemsep{0pt}
  \item 26 informal specification documents
  \item 66 Lean~4 source files ($\approx$19{,}000 lines)
  \item 647 machine-checked theorems
  \item 524 compile-time \texttt{\#guard} correspondence assertions
...
infrastructure.  The twelve-layer architecture emerged organically from
...
Finding no implementation bugs in 647~theorem-checks is itself evidence
...
decisions---stdlib-only (no Mathlib), the twelve-layer decomposition, the
...
\emph{diverges}: the Lean Squad agent completed all 647~theorems
...
a twelve-layer proof architecture, wrote all specifications and proofs,
and produced 647 machine-checked theorems across 66 files in 108 runs,
...
reachability model, and an additional 24 correspondence-validation
files providing 524 compile-time \texttt{\#guard} assertions and 18+
...
specifications, all 647 proofs, the twelve-layer architecture, and the

...
targets, designed an eight-layer proof architecture (data structures, quorum arithmetic,
protocol operations, cross-module composition, Raft safety, reachability,
election model, and correspondence validation), wrote all specifications, and completed all proofs:
524~theorems across 44~Lean~4 files (stdlib only, no Mathlib),
...
reachability model.  An additional 11 correspondence-validation files
provide 189 compile-time \texttt{\#guard} assertions and 11 Rust test
functions, confirming that the Lean models agree with the Rust
implementation on concrete inputs across all major targets.

The total LLM token cost was approximately \$406 (58~runs at
...
the eight-layer proof architecture, specification writing, or any actual
proof engineering.  His total intervention over the project's 57~runs
...
\item The entire effort is \emph{automated}: every target selection,
  ...
  LLM-driven FV agent across 57 independent runs, with only two brief
...
runs: the eight-layer architecture, the choice of targets, and the
...
5 & Raft Safety         & 2  & 20  \\
6 & Reachability        & 1  & 5   \\
7 & Election Model (complete) & 10 & 120 \\
8 & Correspondence Validation & 11 & 0+189\texttt{g} \\
\midrule
  & \textbf{Total}      & \textbf{44} & \textbf{524+189\texttt{g}} \\
\bottomrule
\end{tabular}
\caption{Proof layers and theorem counts. Layer 8 files contain 189 \texttt{\#guard} assertions (\texttt{g}) evaluated at compile time; they do not contribute named theorems.}
\label{tab:layers}
\end{table}
...
To ground the abstract models, we developed 11 \emph{correspondence-test}
...
independent confirmation.  Together, the 189 Lean \texttt{\#guard}
assertions and 11 Rust test functions provide two-sided behavioural
...
Eleven correspondence-test files provide compile-time evidence that the
...
The 11 files cover: \texttt{find\_conflict} (27~guards, exact),
...
(12~guards, exact), \texttt{vote\_result} (17~guards, exact), and
\texttt{has\_quorum} (17~guards, exact).  In total: 189 compile-time
correspondence assertions and 11 Rust test functions.

\subsection{Proof Statistics}

Table~\ref{tab:inventory} summarises the full file inventory.
Total: 524 theorems, 44~files (33 proof files + 11 correspondence-test files),
0 \texttt{sorry}, $\approx$12{,}900 lines of Lean.
...
\midrule
\textbf{Total}    & \textbf{524} & \textbf{0 sorry; 189 \texttt{\#guard}} \\
\bottomrule
\end{tabular}
...
All 524~theorems are consistent with the Rust implementation, as further
confirmed by 189 compile-time \texttt{\#guard} correspondence assertions
and 11 Rust test functions.
...
The eight-layer architecture ensures that every theorem in upper layers has
...
26 FV targets, the eight-layer proof architecture, all 26 informal
specifications, all Lean formalisations, all 524 proofs, the 11
...
Across 58 runs spanning $\approx$10 weeks, the agent produced:
\begin{itemize}\setlength\itemsep{0pt}
  \item 26 informal specification documents
  \item 44 Lean~4 source files ($\approx$12{,}900 lines)
  \item 524 machine-checked theorems
  \item 189 compile-time \texttt{\#guard} correspondence assertions
  \item 11 Rust correspondence test functions
...
infrastructure.  The eight-layer architecture emerged organically from
...
Finding no implementation bugs in 524~theorem-checks is itself evidence
...
Each of the 57 Lean Squad runs consumed approximately \$7 of LLM
...
decisions---stdlib-only (no Mathlib), the eight-layer decomposition, the
...
\emph{diverges}: the Lean Squad agent completed all 524~theorems
...
Squad accumulates knowledge across 57 independent GitHub Actions runs
...
Our $\approx$12{,}900 lines of Lean and 524 theorems are modest by comparison.
...
58 LLM agent runs (each $\sim$15--30 minutes of wall-clock time)
...
an eight-layer proof architecture, wrote all specifications and proofs,
and produced 524 machine-checked theorems across 44 files in 58 runs,
...
reachability model, and an additional 11 correspondence-validation
files providing 189 compile-time \texttt{\#guard} assertions and 11
...
specifications, all 524 proofs, the eight-layer architecture, and the

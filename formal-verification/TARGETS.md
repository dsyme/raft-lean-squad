...
| 6 | `Progress` state machine | `src/tracker/progress.rs`, `src/tracker/state.rs` | 5 — Proofs | ✅ Done | Informal spec + Lean 4 spec + full implementation model (0 `sorry`). ~45 theorems proved covering `maybeUpdate`, all state transitions, `maybeDecrTo`, and the master invariant `valid_preserved_by_all_ops`. See `FVSquad/Progress.lean`. |
...
| 8 | `is_up_to_date` + `find_conflict_by_term` — Log Ordering | `src/raft_log.rs` | 4 — Implementation | 🔄 In progress | Informal spec + Lean 4 spec + implementation model (0 `sorry`). `IsUpToDate` total preorder (8 theorems) + `findConflictByTerm` structural model (7 theorems). See `FVSquad/LogOrdering.lean`. |

...
safe-outputs:
  messages:
    footer: "> Generated by 📐 Lean Squad at [{run-started}]({run_url}). [Learn more](https://github.com/githubnext/agentics/blob/main/docs/repo-assist.md)."
    run-started: "Lean Squad is processing {event_type}, started at at [{run-started}]({run_url})..."
    run-success: "✓ 📐Lean Squad completed successfully at {run-ended}."
    run-failure: "✗ 📐Lean Squad encountered {status} at {run-ended}."
  create-issue:
